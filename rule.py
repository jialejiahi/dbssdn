...
class Chain_hprlist():
    ...
    def add_sw_path_node(self, data_id, in_port, out_port):
        node = {"type": "switch", "dpid": data_id,
                "in": in_port, "out": out_port}
        self.add_pnode(node)
...
class Rule():
    ...
    def construct_rule_action(self, out_port, field_src, value_src,
        ...
        if field_src is not None:
            act.append(
                {"type": "SET_FIELD", "field": field_src, "value": value_src})
        if field_dst is not None:
            act.append(
                {"type": "SET_FIELD", "field": field_dst, "value": value_dst})
            ...
            if first_ovs_id == ep.get_sw_id():
                ...
                hprlist.add_sw_path_node(ep.get_sw_id(),
                                         ...
                                         topo.get_softsw_portno_by_mac(ep.get_sw_id(),
                                                                       ep.get_in_mac()))
            hprlist.add_ep_path_node(
                ep.get_id(), ep.get_in_mac(), ep.get_out_mac())
        else:
            if first_ovs_id == ep.get_sw_id():
                hprlist.add_sw_path_node(first_ovs_id,
                                         topo.get_softsw_uplink_portno(
    ...
    def add_vpc_chain_rules(self, vpc, chain, topo):
        ...
        vprlist_ing = self.calc_rule_chain_one_dire(
            "ingress", vpc, topo, chain)
