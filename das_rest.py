...
def das_get_method(method):
    def wrapper(self, req, *args, **kwargs):
        ...
        # Invoke StatsController method
        try:
            ret = method(self, req, *args, **kwargs)
            return Response(content_type='application/json',
                            body=json.dumps(ret))
        except ValueError:
            LOG.exception('Invalid syntax: %s', req.body)
...
def das_set_method(method):
    def wrapper(self, req, *args, **kwargs):
        ...
        # Invoke StatsController method
        try:
            ret = method(self, req, body, *args, **kwargs)
            return Response(status=200)
        except ValueError:
            LOG.exception('Invalid syntax: %s', req.body)
            return Response(status=400)
...
class StatsController(ControllerBase):
    ...
    @das_set_method
    def switch_add(self, req, cfg, id, **kwargs):
        self.rule.topo.add_softsw(int(str(id), 0), cfg)

    @das_set_method
    def switch_del(self, req, cfg, id, **kwargs):
        self.rule.topo.del_softsw(int(str(id), 0))

    @das_set_method
    def switch_modify(self, req, cfg, id, **kwargs):
        self.rule.topo.set_softsw(int(str(id), 0), cfg)
    ...
    def host_add(self, req, cfg, id, **kwargs):
        self.rule.topo.add_host(int(str(id), 0), cfg)

    @das_set_method
    def host_del(self, req, cfg, id, **kwargs):
        self.rule.topo.del_host(int(str(id), 0))

    @das_set_method
    def host_modify(self, req, cfg, id, **kwargs):
        self.rule.topo.set_host(int(str(id), 0), cfg)
...
class RestStatsApi(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rule.load_config_from_file()
        ...
        uri = path + '/topo/switch/add/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='switch_add',
                       conditions=dict(method=['POST']))

        uri = path + '/topo/switch/del/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='switch_del',
                       conditions=dict(method=['POST']))

        uri = path + '/topo/switch/modify/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='switch_modify',
                       conditions=dict(method=['POST']))

        uri = path + '/topo/switch/lookup/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='switch_lookup',
                       conditions=dict(method=['GET', 'POST']))
 
        uri = path + '/topo/host/add/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='host_add',
                       conditions=dict(method=['POST']))

        uri = path + '/topo/host/del/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='host_del',
                       conditions=dict(method=['POST']))

        uri = path + '/topo/host/modify/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='host_modify',
                       conditions=dict(method=['POST']))

        uri = path + '/host/lookup/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='host_lookup',
                       conditions=dict(method=['GET', 'POST']))
 
        uri = path + '/user/add/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='user_add',
                       conditions=dict(method=['POST']))

        uri = path + '/user/del/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='user_del',
                       conditions=dict(method=['POST']))

        uri = path + '/user/modify/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='user_modify',
                       conditions=dict(method=['POST']))


        uri = path + '/user/lookup/{id}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='user_lookup',
                       conditions=dict(method=['GET', 'POST']))
